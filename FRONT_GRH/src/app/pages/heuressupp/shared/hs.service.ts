...
import { Table, SearchResult } from './hs.model';

import { tableData } from './data';

import { SortDirection } from './hs-sortable.directive';
 ...
 */
 function matches(tables: Table, term: string, pipe: PipeTransform) {
    return pipe.transform(tables.matricule_employe).includes(term)
        || tables.nom.toLowerCase().includes(term)
        || tables.date.toLowerCase().includes(term)
        || pipe.transform(tables.taux_horaire).includes(term)
        || pipe.transform(tables.hs_nuit).includes(term);
...
//Api
addhs(hs,exercice){
    return this.http.post("http://localhost:49366/hs/add/"+exercice,hs);
}

Geths(exercice):Observable<any>{
    return this.http.get<any>("http://localhost:49366/hs/"+exercice);
}

deletehs(id:any):Observable<any>{
        return this.http.delete<any>("http://localhost:49366/hs/delete/"+id);  
}
updatehs(hs,id){
    return this.http.put<any>("http://localhost:49366/hs/update/"+id,hs);
}
