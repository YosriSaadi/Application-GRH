...
import { Table, SearchResult } from './absence.model';

import { tableData } from './data';

import { SortDirection } from './absence-sortable.directive';
...
function matches(tables: Table, term: string, pipe: PipeTransform) {
    return tables.motif.toLowerCase().includes(term)
        ||tables.nom.toLowerCase().includes(term)
        || tables.date_debut.toLowerCase().includes(term)
        || tables.date_fin.toLowerCase().includes(term)
        || tables.mois.toLowerCase().includes(term)
        || pipe.transform(tables.matricule_employe).includes(term)
        || pipe.transform(tables.nbr_jours).includes(term)
...
export class absenceService {
    ...
    addabsence(absence,exercice){
    return this.http.post("http://localhost:49366/absences/add/"+exercice,absence);
    }
    Getmotifs(s):Observable<any>{
        return this.http.get<any>("http://localhost:49366/motifs/"+s);
    }
    Getabsences(exercice):Observable<any>{
        return this.http.get<any>("http://localhost:49366/absences/"+exercice);
        }
    Deletabsence(id:any):Observable<any>{

            return this.http.delete<any>("http://localhost:49366/absence/delete/"+id);  
    }
    ...
    updateabsence(absence,id){  
        return this.http.put<any>("http://localhost:49366/absence/update/"+id,absence);
       }
