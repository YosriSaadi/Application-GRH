...
import { Table, SearchResult } from '../shared/retenue.model';
import { tableData } from '../shared/data';
import { SortDirection } from '../shared/retenue-sortable.directive';
interface State {
    page: number;
    pageSize: number;
    searchTerm: string;
...
function matches(tables: Table, term: string, pipe: PipeTransform) {
    return tables.designation.toLowerCase().includes(term)
        || tables.matricule_employe.toLowerCase().includes(term)
...
export class RetenueService {
     ...
     */
     private _search(): Observable<SearchResult> {
        ...
        // 1. sort
        let tables = sort(this.r, sortColumn, sortDirection);
    ...
    addretenue(exercice,retenue){
        return this.http.post("http://localhost:49366/retenue/add/"+exercice,retenue);
    }
    
    Getretenues(exercice):Observable<any>{
        return this.http.get<any>("http://localhost:49366/retenues/"+exercice);
    }
    Updateretenue(retenue,id){
        return this.http.put<any>("http://localhost:49366/retenue/update/"+id,retenue);
    }
    deleteretenue(id:any):Observable<any>{
        return this.http.delete<any>("http://localhost:49366/retenue/delete/"+id);  
    }
    Get_employe_matricule(matricule):Observable<any>{
    return this.http.get<any>("http://localhost:49366/employee/matricule/"+matricule);
    }
    Gettyperetenues():Observable<any>{
        return this.http.get<any>("http://localhost:49366/type_retenus");
    }
